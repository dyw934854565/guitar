(function() {(function (root, factory) {
  ...
  var _bundleExports = undefined;NovaExports.__fixedUglify="script>";NovaExports.exports={"stylesheet":":host{display:block}","template":"\r\n    "};
        ...
        Nova.Components.NovaSwipable = NovaExports({
            is: 'nova-swipable',
            props: {
            ...
            refresh: function() {
                var self = this;
                Nova.ready([self.$element[0], self.$view[0]], function() {
                    self.min = self.isVertical ? self.$element.height() - self.$view.height() : self.$element.width() - self.$view.width();
                    self.max = 0;

                    if(self.offset !== undefined) {
                        if(self.offset > self.max) {
                            self._scroll(self.max);
                        } if(self.offset < self.min) {
                            self._scroll(self.min);
                        }
                    }
                });
            },
